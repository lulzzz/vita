...
{
  public class TextClassifier : ITextClassifier,IClassifyWho,IClassifyWhat,IClassifyWhere,IClassifyWhen,IClassifyWhy, IClassifyHow
  {
    ...
    public async Task<IEnumerable<TextClassificationResult>> MatchMany(string sentence)
    {
      ...
      do
      {
        var result = GetResult(_results.Any());
    ...
    public DateTime? When(string sentence =  null)
    {
      ...
      if (results.Count > 0 && results.First().TypeName.StartsWith("datetimeV2"))
      {
        ...
        if (subType.Contains("date") && subType.Contains("range") && resolutionValues.Any(x=>x.ContainsKey("start")))
        {
          // range
    ...
    public Classifier Why(string sentence = null)
    {
      ...
      if (_results != null && _results.Any())
        ...
        {
          string Selector(TextClassificationResult y) => y.Classifier?.SubCategory;
    ...
    private static Expression<Func<Company, bool>> SearchCriteriaCompany(string text)
    {
      return company => company.CompanyName.ToLowerInvariant().Contains(text.ToLowerInvariant()) ||
                        (!string.IsNullOrWhiteSpace(company.CurrentName) && company.CurrentName.Contains(text.ToLowerInvariant()));
    }
    ...
    public Locality Where(string sentence)
    {
      ...
      if (!string.IsNullOrWhiteSpace(locality.Postcode)) locality.AustralianState = FindAustralianState().GetValueOrDefault();
