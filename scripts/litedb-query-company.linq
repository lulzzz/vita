...
void Main()
{
	var db = new Database<Company>();
	var data = db.Find(x=>x.Status =="REGD");
	data.Dump();
...
public class Database<T>
{
	private string GetPath() {
		//string type = "company-full"; //{typeof(T).Name}
		string path = $@"D:\Dropbox\Dropbox (Personal)\Red-Trout\chargeid\data\company.db";
	...
	{
		using (var db2 = new LiteDatabase(GetPath()))
		{
